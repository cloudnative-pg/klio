...
# shared OTel collector, both of which live in "default". cert-manager's
# Issuer resources are namespace-scoped, so the client certificate for
# cluster-c has to be generated in "default" (where the klio-server-ca Issuer
# lives, see cluster_c_klio_client_auth.yaml) and then copied here; the
# server/collector certificates are pinned by exact bytes rather than
# validated through a CA (see core/internal/client/klioclient/grpcclient/connection.go),
# so only their public half needs copying.
#
...
kubectl wait --for=create secret/cluster-c-klio-user -n "${SOURCE_NS}" --timeout=120s
...
echo "Copying cluster-c's client certificate (full secret, incl. private key)..."
kubectl get secret cluster-c-klio-user -n "${SOURCE_NS}" -o json |
