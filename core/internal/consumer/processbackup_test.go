...
func TestProcessBackup(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "tier1-only backup verifies, no relay",
		...
		{
			name:         "tier2 relay failure is retried",
			sendToTier2:  true,
			tier2Enabled: true,
			manifests:    someEntries,
			relayErr:     errBoom,
		...
		{
			name:         "tier2 requested but not configured fails immediately",
			sendToTier2:  true,
			tier2Enabled: false,
			manifests:    someEntries,
			wantErr:      true,
